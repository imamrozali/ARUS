// examples/orm/mysql-example.js - Complete ARUS ORM Example with MySQL

import { table, DB, MysqlAdapter, transaction } from '@arusjs/orm';
...
// Setup DB (change to your MySQL instance)
const db = new DB(new MysqlAdapter({
  host: 'localhost',
  port: 3306,
  database: 'testdb',
  user: 'root',
...
console.log('=== ARUS ORM MySQL Example ===\n');

// 1. Create tables (raw SQL for setup)
await db.adapter.execute('CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), email VARCHAR(255), age INT)');
await db.adapter.execute('CREATE TABLE IF NOT EXISTS posts (id INT AUTO_INCREMENT PRIMARY KEY, userId INT, title VARCHAR(255), content TEXT)');
