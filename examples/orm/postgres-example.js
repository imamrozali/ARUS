// examples/orm/postgres-example.js - Complete ARUS ORM Example with PostgreSQL

import { table, DB, PostgresAdapter, transaction } from '@arusjs/orm';
...
// Setup DB (change connection string to your PostgreSQL instance)
...
console.log('=== ARUS ORM PostgreSQL Example ===\n');

// 1. Create tables (raw SQL for setup)
await db.adapter.execute('CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, name TEXT, email TEXT, age INTEGER)');
await db.adapter.execute('CREATE TABLE IF NOT EXISTS posts (id SERIAL PRIMARY KEY, userId INTEGER, title TEXT, content TEXT)');
